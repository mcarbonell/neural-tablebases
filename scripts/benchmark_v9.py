...
from model.models_v9 import VanguardV9, build_giant_graph
...
class NeuralEvaluator:
    def __init__(self, model_path, device="cpu"):
        ...
        self.model = VanguardV9().to(self.device)
        
        print(f"Loading Vanguard V9 weights from {model_path}...")
    ...
    def evaluate_batch(self, boards: List[chess.Board]) -> List[int]:
        ...
        for board in boards:
            # Vanguard V9: Direct Perspective (No mirror)
            # The network was trained on both perspectives (Side To Move).
            p_ids, tac, edges, cnt = self.engine.get_raw_features(board.fen())
            ...
            batch_tac.append(torch.from_numpy(tac.astype(np.float32))) # Normalized /8 in V9 model.forward
        ...
        try:
            p_ids_t = torch.stack(batch_pids).to(self.device).long()
            tac_t = torch.stack(batch_tac).to(self.device).float()
            ...
            # --- DEBUG: Verification of First Board ---
            if not hasattr(self, "_debug_count"):
                self._debug_count = 0
            
            if self._debug_count < 5:
                # Print info for head of batch
                print(f"DEBUG BOARD {self._debug_count}: {boards[0].fen()}")
                # out_wdl[0] scores
                temp_out, _ = self.model(flat_pids[:64], flat_tac[:64], all_srcs[all_srcs < 64], all_dsts[all_dsts < 64], all_etypes[all_srcs < 64], 1)
                probs = torch.softmax(temp_out, dim=1)[0]
                print(f"  Probs [L, D, W]: {probs.tolist()}")
                self._debug_count += 1

            out_wdl, _ = self.model(flat_pids, flat_tac, all_srcs, all_dsts, all_etypes, B)
            
            if torch.isnan(out_wdl).any():
                print("DEBUG: CRITICAL - NaNs found in out_wdl!")
            ...
            import traceback
            print(f"DEBUG: CRITICAL ERROR IN NEURAL EVALUATION:\n{traceback.format_exc()}")
            raise e
