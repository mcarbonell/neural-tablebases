...
impl BitboardBoard {
    ...
    pub fn fill_gnn_features(&mut self, out: &mut crate::GnnFeatureStruct) {
        ...
        // 1. Piece IDs and Node Features
        for sq in 0..64 {
            let bit = 1u64 << sq;
            let mut p_id = 0;
            if (self.occ[0] & bit) != 0 {
                for pt in 1..=6 {
                    if (self.pieces[0][pt] & bit) != 0 { p_id = pt as i8; break; }
                }
            } else if (self.occ[1] & bit) != 0 {
                for pt in 1..=6 {
                    if (self.pieces[1][pt] & bit) != 0 { p_id = (pt + 6) as i8; break; }
                }
            }
            out.piece_ids[sq] = p_id;

            // Attacker Counts and Bitmasks
            let w_atks = self.get_attackers(sq, WHITE, occ);
            let b_atks = self.get_attackers(sq, BLACK, occ);
            
            out.white_atk_counts[sq] = w_atks.0 as i8;
            out.black_atk_counts[sq] = b_atks.0 as i8;
            out.attacker_bitmasks[sq] = (w_atks.1 | b_atks.1) as u8; // Combine bitmasks OR keep separate? 
            // Spec said u8 bitmask, usually piece types. Let's make it more granular if needed.
        }

        // 2. Flags: Check, Pin, King Oxygen
        let king_sq_side = self.pieces[side][KING as usize].trailing_zeros() as usize;
        let king_sq_opp = self.pieces[opp][KING as usize].trailing_zeros() as usize;

        if king_sq_side < 64 && self.is_attacked(king_sq_side, opp) {
            out.flags[king_sq_side] |= 2; // Bit 1: Checked
        }

        // 3. Multi-Channel Edges
        // Channel 0: Mobility (Strictly Legal)
        let legal_moves = self.generate_moves_internal();
        self.add_edges(out, &legal_moves, 0);

        // Channel 1: Influence (Pseudo-legal + Self-defense)
        self.add_influence_edges(out, WHITE);
        self.add_influence_edges(out, BLACK);

        // Channel 2: X-Ray
        self.add_xray_edges(out);
    }

    fn get_attackers(&self, sq: usize, side: usize, occ: u64) -> (u32, u8) {
        let mut count = 0;
        let mut mask = 0;
        
        let p_atks = p_atk_to(1-side, 1u64 << sq) & self.pieces[side][PAWN as usize];
        if p_atks != 0 { count += p_atks.count_ones(); mask |= 1 << (PAWN-1); }

        let n_atks = knight_attacks(sq) & self.pieces[side][KNIGHT as usize];
        if n_atks != 0 { count += n_atks.count_ones(); mask |= 1 << (KNIGHT-1); }

        let b_atks = bishop_attacks(sq, occ) & (self.pieces[side][BISHOP as usize] | self.pieces[side][QUEEN as usize]);
        if b_atks != 0 { count += b_atks.count_ones(); mask |= 1 << (BISHOP-1); }

        let r_atks = rook_attacks(sq, occ) & (self.pieces[side][ROOK as usize] | self.pieces[side][QUEEN as usize]);
        if r_atks != 0 { count += r_atks.count_ones(); mask |= 1 << (ROOK-1); }

        let k_atks = king_attacks(sq) & self.pieces[side][KING as usize];
        if k_atks != 0 { count += k_atks.count_ones(); mask |= 1 << (KING-1); }

        (count, mask)
    }

    fn add_edges(&self, out: &mut crate::GnnFeatureStruct, moves: &[Move], edge_type: u16) {
        for m in moves {
            if out.edge_count >= 1024 { break; }
            let edge = (edge_type << 12) | ((m.from as u16) << 6) | (m.to as u16);
            out.edges[out.edge_count as usize] = edge;
            out.edge_count += 1;
        }
    }

    fn add_influence_edges(&self, out: &mut crate::GnnFeatureStruct, side: usize) {
        let occ = self.occ[2];
        let our_occ = self.occ[side];
        ...
        let king_sq = self.pieces[side][KING as usize].trailing_zeros() as usize;
        if king_sq < 64 {
            let atk = king_attacks(king_sq);
            self.add_edge_mask(out, king_sq, atk, 1);
        }
        // Knight
        let mut knights = self.pieces[side][KNIGHT as usize];
        while knights != 0 {
            let sq = knights.trailing_zeros() as usize;
            knights &= knights - 1;
            let atk = knight_attacks(sq);
            self.add_edge_mask(out, sq, atk, 1);
        }
        // Slider
        let slider_b = self.pieces[side][BISHOP as usize] | self.pieces[side][QUEEN as usize];
        let mut bishops = slider_b;
        while bishops != 0 {
            let sq = bishops.trailing_zeros() as usize;
            bishops &= bishops - 1;
            let atk = bishop_attacks(sq, occ);
            self.add_edge_mask(out, sq, atk, 1);
        }
        let slider_r = self.pieces[side][ROOK as usize] | self.pieces[side][QUEEN as usize];
        let mut rooks = slider_r;
        while rooks != 0 {
            let sq = rooks.trailing_zeros() as usize;
            rooks &= rooks - 1;
            let atk = rook_attacks(sq, occ);
            self.add_edge_mask(out, sq, atk, 1);
        }
        // Pawn attacks
        let pawns = self.pieces[side][PAWN as usize];
        if side == WHITE {
            let l_cap = (pawns << 7) & NOT_H_FILE;
            let r_cap = (pawns << 9) & NOT_A_FILE;
            self.add_edge_mask_pawn(out, side, 7, l_cap, 1);
            self.add_edge_mask_pawn(out, side, 9, r_cap, 1);
        } else {
            let l_cap = (pawns >> 9) & NOT_H_FILE;
            let r_cap = (pawns >> 7) & NOT_A_FILE;
            self.add_edge_mask_pawn(out, side, -9, l_cap, 1);
            self.add_edge_mask_pawn(out, side, -7, r_cap, 1);
        }
    }

    fn add_xray_edges(&self, out: &mut crate::GnnFeatureStruct) {
        // Simple X-Ray: For each slider, remove first blocker and get new attacks
        let occ = self.occ[2];
        for side in 0..2 {
            let bishops = self.pieces[side][BISHOP as usize] | self.pieces[side][QUEEN as usize];
            let mut b = bishops;
            while b != 0 {
                let sq = b.trailing_zeros() as usize;
                b &= b - 1;
                let direct = bishop_attacks(sq, occ);
                let first_blockers = direct & occ;
                let xray = bishop_attacks(sq, occ ^ first_blockers);
                self.add_edge_mask(out, sq, xray ^ direct, 2);
            }
            let rooks = self.pieces[side][ROOK as usize] | self.pieces[side][QUEEN as usize];
            let mut r = rooks;
            while r != 0 {
                let sq = r.trailing_zeros() as usize;
                r &= r - 1;
                let direct = rook_attacks(sq, occ);
                let first_blockers = direct & occ;
                let xray = rook_attacks(sq, occ ^ first_blockers);
                self.add_edge_mask(out, sq, xray ^ direct, 2);
            }
        }
    }

    fn add_edge_mask(&self, out: &mut crate::GnnFeatureStruct, from: usize, mut atk: u64, edge_type: u16) {
        while atk != 0 {
            if out.edge_count >= 1024 { break; }
            let to = atk.trailing_zeros() as usize;
            atk &= atk - 1;
            let edge = (edge_type << 12) | ((from as u16) << 6) | (to as u16);
            out.edges[out.edge_count as usize] = edge;
            out.edge_count += 1;
        }
    }

    fn add_edge_mask_pawn(&self, out: &mut crate::GnnFeatureStruct, _side: usize, offset: i32, mut dests: u64, edge_type: u16) {
        while dests != 0 {
            if out.edge_count >= 1024 { break; }
            let to = dests.trailing_zeros() as usize;
            dests &= dests - 1;
